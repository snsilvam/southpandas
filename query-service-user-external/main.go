...
func newRouter() (router *mux.Router) {
	...
	router.HandleFunc("/users-external-workers", listUserExternalWorkerHandler).Methods(http.MethodGet)
	router.HandleFunc("/users-external-workers-search", searchHandler).Methods(http.MethodGet)
	return
}
...
func main() {
	...
	repo, err := database.NewPostgresRepositoryUserExternalWorker(addr)
	if err != nil {
		log.Fatal(err)
	}
	repository.SetRepositoryUserExternalWorker(repo)
	...
	err = n.OnCreateUserExternalWorker(onCreatedUserExternalWorker)
	if err != nil {
		log.Fatal(err)
	}
